...
** Returns:  NOne
*/
void periph_WriteByte( const UINT8 CoProWriteData )
{
...
**
*/
void periph_WriteBytes( const UINT8 * data,  UINT16 numBytes)
{
   UINT16 i;

   for (i = 0; i < numBytes; i++)
   {
      periph_WriteByte( data[i] );
   }
}
...
*/
void periph_ReadByte( UINT8 *ByteLocation )
{
...
*/
void periph_ReadBytesAck( UINT8 *BytesLocation, UINT8 ByteCount )
{
   UINT8 i;
   ...
   {
      /* read next byte */
      periph_ReadByte( &BytesLocation[i] );
...
** Returns:  BytesLocation read bytes
*/
void periph_ReadBytes( UINT8 *BytesLocation, UINT8 ByteCount, BOOL Ack )
{
   UINT8 i;
