...
/* -----------------------------------------------------------------------------
 * Function: int addHeadings(int heading1, int heading2)
 * 
 * Adds together two headings, and scales the answer to be between -180 and 180
 */
int addHeadings(int heading1, int heading2)
{
    int tempHeading = heading1 + heading2;
    
    if (tempHeading >= -180 && tempHeading <= 180){
        return tempHeading;
    }
    if (tempHeading < -180){
...
void readCompassData()
{
    ...
    X_axis = createIntFromChars(data[0], data[1]);  // MSB,LSB
    Z_axis = createIntFromChars(data[2], data[3]);  // MSB,LSB
    Y_axis = createIntFromChars(data[4], data[5]);  // MSB,LSB
};  
 ...
 * Function: getHeading();
 * 
 * Reads the compass in single measurement mode, gets data, calculates current heading.
 * 
 * INPUTS: none
 * 
 * OUTPUT: int current heading
 */
int getHeading()
{ 
...
//    temp_heading *= (180.0 / PI);*/
    ...
    signed int degrees = atan_int(X_axis, Y_axis);
...
{
    int heading = atan_int(X_axis,Y_axis);
    return heading;
}